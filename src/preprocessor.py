...
from .utils import (
    ...
    seq_window_mean, seq_error_streak
)
...
class Preprocessor:
    ...
    def preprocess_a(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        feats["A1_rt_window_early"] = seq_window_mean(df["A1-4"], window='early')
        feats["A1_rt_window_mid"] = seq_window_mean(df["A1-4"], window='mid')
        feats["A1_rt_window_late"] = seq_window_mean(df["A1-4"], window='late')
        ...
        # Error patterns
        feats["A1_error_streak"] = seq_error_streak(df["A1-3"], target="0")
        ...
        # Window-based features
        feats["A2_rt_window_early"] = seq_window_mean(df["A2-4"], window='early')
        feats["A2_rt_window_late"] = seq_window_mean(df["A2-4"], window='late')
        
        # Error patterns
        feats["A2_error_streak"] = seq_error_streak(df["A2-3"], target="0")
        ...
        # Window-based features
        feats["A3_rt_window_early"] = seq_window_mean(df["A3-7"], window='early')
        feats["A3_rt_window_late"] = seq_window_mean(df["A3-7"], window='late')
        ...
        # Window-based features
        feats["A4_rt_window_early"] = seq_window_mean(df["A4-5"], window='early')
        feats["A4_rt_window_mid"] = seq_window_mean(df["A4-5"], window='mid')
        feats["A4_rt_window_late"] = seq_window_mean(df["A4-5"], window='late')
        
        # Error patterns
        feats["A4_error_streak"] = seq_error_streak(df["A4-3"], target="0")
        ...
        # Error patterns
        feats["A5_error_streak"] = seq_error_streak(df["A5-2"], target="0")
    ...
    def preprocess_b(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Window-based features
        feats["B1_rt_window_early"] = seq_window_mean(df["B1-2"], window='early')
        feats["B1_rt_window_late"] = seq_window_mean(df["B1-2"], window='late')
        ...
        # Error patterns
        feats["B1_error_streak_task1"] = seq_error_streak(df["B1-1"], target="0")
        feats["B1_error_streak_task2"] = seq_error_streak(df["B1-3"], target="0")
        ...
        # Window-based features
        feats["B2_rt_window_early"] = seq_window_mean(df["B2-2"], window='early')
        feats["B2_rt_window_late"] = seq_window_mean(df["B2-2"], window='late')
        ...
        # Error patterns
        feats["B2_error_streak_task1"] = seq_error_streak(df["B2-1"], target="0")
        feats["B2_error_streak_task2"] = seq_error_streak(df["B2-3"], target="0")
        ...
        # Window-based features
        feats["B3_rt_window_early"] = seq_window_mean(df["B3-2"], window='early')
        feats["B3_rt_window_late"] = seq_window_mean(df["B3-2"], window='late')
        
        feats["B3_acc_consistency"] = seq_consistency(df["B3-1"])
        feats["B3_acc_trend"] = seq_trend(df["B3-1"])
        
        # Error patterns
        feats["B3_error_streak"] = seq_error_streak(df["B3-1"], target="0")
        ...
        # Window-based features
        feats["B4_rt_window_early"] = seq_window_mean(df["B4-2"], window='early')
        feats["B4_rt_window_late"] = seq_window_mean(df["B4-2"], window='late')
        
        feats["B4_acc_consistency"] = seq_consistency(df["B4-1"])
        feats["B4_acc_trend"] = seq_trend(df["B4-1"])
        
        # Error patterns
        feats["B4_error_streak"] = seq_error_streak(df["B4-1"], target="0")
        ...
        # Window-based features
        feats["B5_rt_window_early"] = seq_window_mean(df["B5-2"], window='early')
        feats["B5_rt_window_late"] = seq_window_mean(df["B5-2"], window='late')
        
        feats["B5_acc_consistency"] = seq_consistency(df["B5-1"])
        feats["B5_acc_trend"] = seq_trend(df["B5-1"])
        
        # Error patterns
        feats["B5_error_streak"] = seq_error_streak(df["B5-1"], target="0")
        ...
        # Error patterns
        feats["B6_error_streak"] = seq_error_streak(df["B6"], target="0")
        feats["B7_error_streak"] = seq_error_streak(df["B7"], target="0")
        feats["B8_error_streak"] = seq_error_streak(df["B8"], target="0")
